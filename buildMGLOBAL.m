function [MGLOBAL] = buildMGLOBAL(NODES,SCTR,DOF,AREA,DENSITY,MGLOBAL,FIXED, DISTRIBUTE_MASS)
...
for el = 1:N
    ...
    if ismember(START, FIXED) == 1
        if DISTRIBUTE_MASS ==  1
            MASS_START = (MASS(el)/2) * eye(DOF);
        else
            MASS_START = (MASS(el)) * eye(DOF);
        end
    end
